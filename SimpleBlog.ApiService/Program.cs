...
sealed class InMemoryPostRepository : IPostRepository
{
    ...
    public InMemoryPostRepository()
    {
        ...
        var seed = new[]
        {
            new Post(Guid.NewGuid(), "Pierwszy wpis", "Witaj w SimpleBlog!", "System", now, ImmutableArray<Comment>.Empty, null),
            new Post(Guid.NewGuid(), "Drugi wpis", "Edytuj lub dodaj nowe posty, aby zobaczyć React w akcji.", "System", now.AddMinutes(-30), ImmutableArray<Comment>.Empty, null)
        };

        foreach (var post in seed)
        {
    ...
    public Post Create(CreatePostRequest request)
    {
        ...
        var post = new Post(Guid.NewGuid(), request.Title.Trim(), request.Content.Trim(), author, DateTimeOffset.UtcNow, ImmutableArray<Comment>.Empty, request.ImageUrl);
        _posts[post.Id] = post;
        return post;
    }
    ...
    public Post? Update(Guid id, UpdatePostRequest request)
    {
        ...
        var updated = existing with
        {
            ...
            Author = string.IsNullOrWhiteSpace(request.Author) ? existing.Author : request.Author.Trim(),
            CreatedAt = existing.CreatedAt,
            Comments = existing.Comments,
            ImageUrl = string.IsNullOrWhiteSpace(request.ImageUrl) ? existing.ImageUrl : request.ImageUrl
        };
    ...
    {
        return _posts.TryGetValue(postId, out var post) ? post.Comments : null;
    }

    public Comment? AddComment(Guid postId, CreateCommentRequest request)
    {
        if (!_posts.TryGetValue(postId, out var existing))
        {
            return null;
        }
        ...
        var comment = new Comment(Guid.NewGuid(), postId, author, content, DateTimeOffset.UtcNow);

        var updatedComments = (existing.Comments as ImmutableArray<Comment>? ?? existing.Comments.ToImmutableArray()).Add(comment);
        var updated = existing with { Comments = updatedComments };
        _posts[postId] = updated;
        return comment;
    }
}
